...
class Option {
	...
	void _load_config () {
		...
		if (this._default_config_file_path.isDir) {
		    ...
		    Log.instance.addWarning ("Utilisation du fichier de configuration par défaut.");
		    try {
	...
	void _load_general_params (Balise b) {
		...
		Log.instance.addInfo ("Paramètre chargé : use_sessid -> ", child.getValue());
		...
		Log.instance.addInfo ("Paramètre chargé : session_dir -> ", b.childs[0].getValue());
		...
		Log.instance.addInfo ("Paramètre chargé : log_file -> ", b.childs[0].getValue());
	    } else if (b.name.name == "debug_mode") {
		if (b.childs.length != 1)
		    throw new ConfigFileError ("Le format du fichier de configuration est incomplet. Détail : log_file_path");
		this._debug_mode = (b.childs[0].getValue() == "true");
		Log.instance.addInfo ("Paramètre chargé : debug_mode -> ", b.childs[0].getValue());
	    } else if (b.name.name == "controllers") {
		if (b.childs.length != 1)
	...
	void _load_controllers (Balise b) {
		...
		if (b.childs.length == 2) {
			...
			Log.instance.addInfo ("Controlleur ajouté : ", b_name.childs[0].getValue(), " -> ", b_class.childs[0].getValue());
		    }
		} else {
		    //TODO
		}
